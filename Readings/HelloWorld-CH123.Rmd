...
- Mechanistic: mechanistic models mean that the data should meet the assumptions the model makes. These are models that are not very responsible to data, they lean heavily to certain assumptions. 
* Assumptions for linear regression: linearity, normality, homoscedasticity (errors generally the same across the line).
- Possible goals: 
* predictive
* descriptive
* inferential (sometimes): how data is generated allows it to be inferential - the data should be randomly generated: random sampling or random assignment
- Perform measures
* RSE (residual standard error)
* $R^2$: how much of the variation/signal of the output (Y) is explained by the model.
* MSE (mean squared error)

b. With supervised models, there are two main sub-categories:

(1) Regression: predicts a numeric/continuous outcome.
...
_a_ Inline functions can be used in the formula:

* `rate ~ log(temp)`

* `rate ~ I((temp * 9/5) + 32)` the identity function `I()` applies literal math to the predictors.

_b_ R has many functions that are useful inside of formulas. 
...
_c_ For data sets where there are many predictors, the period shortcut is available. The period represents main effects for all of the columns that are not on the left-hand side of the tilde. 

* `~ (.)^3` creates main effects as well as all two- and three- variables interactions to the model. 


(2) Fit a two-way interaction model to chirping crickets:
